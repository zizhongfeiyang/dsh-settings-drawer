// Browser client half of dsh-settings-drawer
...
window.__ModuleLoader__.load({ id: 'dsh-settings-drawer', factory: (require) => {
  ...
  function defaultConfig() {
    const cfg = { enableDrawer: true, visibleCards: {} }
  ...
  function getSavedConfig() {
    try {
      ...
      if (raw) {
        ...
        return {
          enableDrawer: parsed.enableDrawer !== false,
  ...
  function hiddenIds(cfg) {
    ...
    const consider = (cardId) => {
      if (!cardId || seen.has(cardId) || cardId === 'settings-drawer-manager') return
      seen.add(cardId)
  ...
  function stampNavButtons() {
    const slots = window.__dshSettingsDrawerSlots
  ...
  function saveConfig(cfg) {
    try {
      ...
      window.dispatchEvent(new CustomEvent('dsh-settings-drawer-updated', { detail: cfg }))
    } catch (e) {}
    notifySettingsNav()
  }
  ...
  function shouldKeep(id, cfg) {
    if (!id) return true
    if (id === 'settings-drawer-manager') return true
    if (!cfg.enableDrawer) return true
  ...
  function ManagerCard() {
    ...
    React.useEffect(() => {
      ...
      const refresh = () => {
        try {
          const raw = slots && typeof slots.entries === 'function'
            ? (slots.__dshSettingsDrawerRawEntries || slots.entries).call(slots, 'settings.section')
            : []
          const rows = (raw || [])
            .map((e) => ({
            ...
            .filter((r) => r.id && r.id !== 'settings-drawer-manager')
      ...
      window.addEventListener('dsh-settings-drawer-updated', refresh)
      return () => {
        clearInterval(timer)
        window.removeEventListener('dsh-settings-drawer-updated', refresh)
      }
    }, [])
    ...
    const toggleDrawer = (e) => {
      const next = { ...cfg, enableDrawer: e.target.checked }
      setCfg(next)
      saveConfig(next)
    }
    ...
    const hideAllExtras = () => {
      const next = { enableDrawer: true, visibleCards: { ...cfg.visibleCards } }
      catalog.forEach((c) => {
    ...
    return h('div', {
      ...
      cfg.enableDrawer && h('div', { key: 'list', style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '8px', marginTop: '4px' } },
        catalog.map(card => {
